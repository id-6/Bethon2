...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@Bethon2"))
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@J_G_H))
    except BaseException:
        pass
...
async def join_channel(event):
    try:
        await sython(JoinChannelRequest("@F_P_l"))
    except BaseException:
        pass  
...
async def _(event):
    ...
    await sython.send_message(bot_usernameeeeee, '/start')
    await asyncio.sleep(4)
...
@sython.on(events.NewMessage(outgoing=True, pattern=".تجميع اكس المليار"))
async def _(event):
...
async def _(event):
      await event.reply("""بيثون : @Bethon2"""
)
